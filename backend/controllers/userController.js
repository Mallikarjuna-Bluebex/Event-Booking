...
const bookingEvent = async (req, res) => {
    try {
      ...
      for (const service of services) {
            ...
            //console.log("option: ",option)//event service
            if (!option) {
...
const updatePaymentStatus = async (req, res) => {
    ...
    try {
      ...
      for (const service of services) {
        ...
        if (!option.bookingsByDate) {
            option.bookingsByDate = new Map();
        }
        const prevBookings = option.bookingsByDate.get(today) || 0;
        option.bookingsByDate.set(today, prevBookings + service.quantity);
