...
const InvoiceTemplate2 = () => {
  ...
  useEffect(() => {
    ...
    const fetchTransaction = async () => {
      try {
        const res = await axios.get(`${backendUrl}/api/admin/get-userinfo/${bookingId}`);
   ...
   useEffect(() => {
       axios
         .get(`${backendUrl}/api/admin/get-invoiceinfo`)
         .then((response) => {
     ...
     console.log("user: ",user)
         ...
         const invoiceData = {
           ...
           date: transaction?.createdAt || new Date().toISOString().split("T")[0], // Current date
           dueDate: transaction?.selectedDate || new Date().toISOString().split("T")[0],
           seller: {
    ...
    const handleSaveTemplate = async() => {
      ...
      try {
        const response = await axios.put(`${backendUrl}/api/admin/update-template`, {
      ...
      <div className="my-auto mr-8 md:mr-15">
      {
              ...
              <label htmlFor="image">
                <div className='inline-block relative cursor-pointer'>
                  <img className='w-44 h-24 rounded opacity-75' src={image ? URL.createObjectURL(image) : user.logo} alt="Uploaded Preview" />
